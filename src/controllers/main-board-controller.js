...
import SortComponent, {SortType} from "../components/sort-template.js";
...
export default class PageController {
  ...
  render(filmCards) {
    ...
    // ------------------------------render-film-card-------------------------
    let renderFilmCard = (place, filmCard) => {
    ...
    this._sortComponent.setSortTypeChangeHandler((sortType) => {
      ...
      const sortedFilmCards = getSortFilmCards(filmCards, sortType, 0, CARDS_COUNT);
    ...
    const renderShowMoreBtn = () => {
      ...
      this._showMoreBtn.setShowMoreBtnClickHandler(() => {
        ...
        const sortedCards = getSortFilmCards(filmCards, this._sortComponent.getSortType(), prevFilmCards, countShowingFilmCards);
        renderFilmCards(sortedCards);

        if (countShowingFilmCards >= filmCards.length) {
          remove(this._showMoreBtn);
        }
      });
    };
    ...
    let sortCards = getSortFilmCards(filmCards, SortType.RATING, 0, CARDS_COUNT_FOR_OTHER);
    sortCards.forEach((filmCard) => {
      renderFilmCard(topRatedContainer, filmCard);
    });
    sortCards = getSortFilmCards(filmCards, SortType.COMMENT, 0, CARDS_COUNT_FOR_OTHER);
    sortCards.forEach((filmCard) => {
      renderFilmCard(topCommentContainer, filmCard);
    });
  }
}
